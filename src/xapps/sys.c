...
unsigned _get_volume_info(const char *path, int *max_file_len, int *max_path_len, char *fsystype) {
    ...
    // Copy the address of the buffer to the transfer buffer (__tb)
    dosmemput(buffer, 32, __tb);

    // Call the interrupt using DPMI
    __dpmi_int(0x21, &regs);

    // Check for errors
    if (regs.x.flags & 1) {  // Check carry flag
        errno = EINVAL;  // Set appropriate errno value
        return _FILESYS_UNKNOWN; // Indicate error
    }

    // Retrieve the data from the transfer buffer
    dosmemget(__tb, 32, buffer);

    // Copy the data to fsystype
    if (fsystype) {
        strncpy(fsystype, buffer, 32);
    }
...
void Commanddf() {
    ...
    for (drive = 3; drive <= 26; drive++) { // Iterate through drives C to Z
        ...
        if (_dos_getdiskfree(drive, &dfree) == 0) {
            ...
            DebugMessage("Volume info for drive %c: %u\n", drive_letter, volume_info); // Print volume_info for debugging
            if (volume_info == _FILESYS_UNKNOWN) {
                DebugMessage("Error retrieving volume information for drive %c\n", drive_letter);
                strcpy(fs_type, "Unknown");
            } else {
                // Map the filesystem type to a more user-friendly string
                const char *fs_type_str = "Unknown";
                switch (volume_info) {
                    case 0: fs_type_str = "Unknown"; break;
                    case 1: fs_type_str = "FAT12"; break;
                    case 2: fs_type_str = "FAT16"; break;
                    case 3: fs_type_str = "FAT32"; break;
                    case 4: fs_type_str = "NTFS"; break;
                    // Add more cases as needed
                    default: fs_type_str = "VFAT"; break;
                }
                DebugMessage("Drive %c: max file length = %d, max path length = %d, filesystem type = %s\n",
                    drive_letter, max_file_len, max_path_len, fs_type_str);

                strncpy(fs_type, fs_type_str, sizeof(fs_type) - 1);
                fs_type[sizeof(fs_type) - 1] = '\0'; // Ensure null-terminated string
            }
