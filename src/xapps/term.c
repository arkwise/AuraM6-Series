...
//  Terminal
//
//  Copyright (c) Aura GUI m6 Terminal. v3
//
////////////////////////////////////////////////////////////////////////////////
...
#ifndef S_IFDIR
#define S_IFDIR 0040000  // This is a common UNIX value, but check DJGPP's documentation testhjhhj
#endif

int mkdir(const char *pathname, mode_t mode); // <-- This was originally in your code.

l_ulong AppVersion = ULONG_ID(0,0,5,8);
...
0062 df has been added to the sys.c
    ...
    do {
        ...
        if (new_argc) {
            if (!TextCaseCompare(new_argv[0], "exit")) {
                ...
                list_cpu_info();
            } else if (!TextCaseCompare(new_argv[0], "df")) {               
                Commanddf();
            } else if (!TextCaseCompare(new_argv[0], "colors")) {
                CommandColors();
            } else {
