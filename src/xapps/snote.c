...
//	Aura Milestone 6 
//
//	(c) Copyright Doscore Aura m6 . All rights reserved.
//
/////////////////////////////////////////////////////////////////////////////////
#include "kernel.h"
#include "widget.h"
...
/*
001 basic updates to try and get this working again via menu.h compiling issues.
*/
...
void GenerateSubMenu(PMenuItem itm, void* Args)
{
	...
	if (TextCompare(arg, "Note") == 0)
	{
		itm->SubMenu = NewMenu(
			NewMenuItem("Remove", NULL, MSG_NOTE_REMOVE, 0, NULL, NULL)
		);
		AddMenuItem(itm->SubMenu, "Hide", NULL, MSG_NOTE_HIDE, 0, NULL);
	}
	else if (TextCompare(arg, "Traybar") == 0)
	{
		itm->SubMenu = NewMenu(
			NewMenuItem("New note", NULL, MSG_TRAY_NEW, 0, NULL, NULL)
		);
		AddMenuItem(itm->SubMenu, "Exit", NULL, MSG_TRAY_EXIT, 0, NULL);
	}
}
...
l_int Main(int argc, l_text *argv)
{
	...
	NOTEMenu = NewMenu(
		NewMenuItemEx("Note", NULL, 0, 0, NULL, &GenerateSubMenu, "Note", NULL)
	);

	PMenu NOTETraybarMenu = NewMenu(
		NewMenuItemEx("Traybar", NULL, 0, 0, NULL, &GenerateSubMenu, "Traybar", NULL)
	);
