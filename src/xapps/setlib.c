...
void GenerateSubMenu(PMenuItem itm, void* Args) {
    ...
    if (TextCompare(arg, "Libraries") == 0) {
        PMenuItem items[] = {
            NewMenuItem("Install a new library", NULL, MSG_REGISTERLIB, 0, NULL, NULL),
            NewMenuItem("Remove selected", NULL, MSG_UNREGISTERLIB, 0, NULL, NULL),
            NULL
        };
        itm->SubMenu = NewMenu(items);
    } else if (TextCompare(arg, "Help") == 0) {
        PMenuItem items[] = {
            NewMenuItem("Informations on startup phases", NULL, 0, 0, NULL, NULL),
            NULL
        };
        itm->SubMenu = NewMenu(items);
    }
...
l_int Main(int argc, l_text *argv) {
    ...
    PMenu Menu = NewMenu(
        NewMenuItemEx("Libraries", NULL, 0, 0, NULL, &GenerateSubMenu, "Libraries", NULL)
    );
