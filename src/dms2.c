...
//#define DebugError(msg, ...) fprintf(stderr, "ERROR: " msg "\n", ##__VA_ARGS__)
//#define DebugWarning(msg, ...) fprintf(stderr, "Warning: " msg "\n", ##__VA_ARGS__)
//#define DebugMessage(msg, ...) fprintf(stdout, "Message: " msg "\n", ##__VA_ARGS__)
...
PSubCodec GetSubCodec ( l_ulong CodecId, l_ulong TypeId ) {
    ...
    if ( !sc ) {
        ...
        if ( dtm ) {
            ...
            if ( sc )
                DebugWarning("DMS2 : Use default sub-codec instead");
            else
...
void *LoadDataEx ( l_text File, l_ulong TypeId, l_ulong CodecId, l_ulong *ExtraLoad, PErrorHandler h ) {
    ...
    if ( CodecId )
        ...
        sc = GetSubCodecFile(File,TypeId);
    if ( !sc || !File) {
        DebugError("DMS2: No subcodec found for file %s and data type %x", File, TypeId);
        return NULL;
    }
